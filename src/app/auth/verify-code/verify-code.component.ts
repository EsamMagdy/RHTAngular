...
export class VerificationCodeComponent implements OnInit {
    ...
    constructor(private authService: AuthService,
        ...
        this.user = this.localStorageService.registrationDataLocalStorage
    ...
    onSubmit() {
        ...
        let verifyCodeObs: Observable<ResponseDataCRMWithObjectData<VerificationCode>>;
    ...
    verifySignInCode(code: string) {
        this.authService.verifySignInCode(code).subscribe(
            (resData) => {
                if (!resData.state && resData.message) {
                    this.error = resData.message[0].value;
                    return;
    ...
    resendCode() {
        let authServiceObs: Observable<ResponseDataCRMWithObjectData<ReGenrateCode>>;
        this.resendCodeSuccess = false;
        this.error = null;
        if (this.user.phoneNumberConfirmed)
            authServiceObs = this.authService.resendSignInCode();
        else
            authServiceObs = this.authService.resendCode();
