...
export class IndOrdersDetailsComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.route.data.subscribe((data: Data) => {
      ...
      this.deservedAmount =
        Math.round(this.individualContractReq?.finalPrice ?? 0) +
        Math.round(this.individualContractReq?.deliveryCost ?? 0);
    });
  ...
  cancelRequest() {
    this.indOrdersService
      ...
      .subscribe((data) => {
        if (data.value) this.router.navigate(['/dashboard/ind-orders']);
      });
  }
}
