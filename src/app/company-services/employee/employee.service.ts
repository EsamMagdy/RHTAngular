...
import { ObjectWithPaging, ResponseDataCRMWithPaging, ResponseDataCRMWithObjectData } from "src/app/shared/models/responseDataCRM.model";
...
export class EmployeeService {
    ...
    getAvaialableEmployees(nationalityId: string, professionId: string, pageSize: number) {
        return this.http
    ...
    getEmployeeById(employeeId: string) {
        return this.http.get<ResponseDataCRMWithObjectData<Employee>>(environment.apiUrl + `Employee/Get/${employeeId}`)
            .pipe(map(resData => {
                return resData.data
            }));
    }
    getemplyeeFilter(employeeFilteringdata: EmployeFilteringData) {
        return this.http.post<ResponseDataCRMWithPaging<Employee>>(environment.apiUrl + `IndividualContractRequest/FilterEmployee`,
